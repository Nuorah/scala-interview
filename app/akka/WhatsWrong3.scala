...
//onComplete might call on a different thread than the actor, akka cannot then garantee in what
//order internalState will be mutated. To fix it when the future is completed send a message to 
//self to then mutate internal state
